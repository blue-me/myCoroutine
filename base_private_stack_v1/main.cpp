...
int main() {
    ...
    // scheduler.addCoroutine(make_unique<Coroutine>(coFib, ptr1));
    // scheduler.addCoroutine(make_unique<Coroutine>(coEcho, ptr1, ptr2));
    for(int i = 0;i<5;++i)
    {
        scheduler.addCoroutine(make_unique<Coroutine>(producer));
    }
    scheduler.addCoroutine(make_unique<Coroutine>(consumer));
